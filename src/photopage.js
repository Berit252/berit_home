...
if (gallery01Images) {
    gallery01Images.forEach((image, index) => {
        image.onclick = () => {
            ...
            let getImgUrlPos = getFullImgUrl.split('/public/img/pack1/small_size/')
            ...
            newImg.setAttribute('src', '../public/img/pack1/' + setNewImgUrl)
...
if (gallery02Images) {
    gallery02Images.forEach((image, index) => {
        image.onclick = () => {
            let getElementCSS = window.getComputedStyle(image)
            let getFullImgUrl = getElementCSS.getPropertyValue('background-image')
            let getImgUrlPos = getFullImgUrl.split('/public/img/pack2/small_size/')
            let setNewImgUrl = getImgUrlPos[1].replace('")', '')
            ...
            let newImg = document.createElement('img')
            newImgWindow.appendChild(newImg)
            newImg.setAttribute('src', '../public/img/pack2/' + setNewImgUrl)
            newImg.setAttribute('id', 'current-img')
...
if (gallery03Images) {
    gallery03Images.forEach((image, index) => {
        image.onclick = () => {
            let getElementCSS = window.getComputedStyle(image)
            let getFullImgUrl = getElementCSS.getPropertyValue('background-image')
            let getImgUrlPos = getFullImgUrl.split('/public/img/pack3/small_size/')
            let setNewImgUrl = getImgUrlPos[1].replace('")', '')
            ...
            let newImg = document.createElement('img')
            newImgWindow.appendChild(newImg)
            newImg.setAttribute('src', '../public/img/pack3/' + setNewImgUrl)
            newImg.setAttribute('id', 'current-img')
